...
# Set reproducibility seeds for reproducibility across runs.
...
#    seed                   - Integer value setting execution seeds for reproducibility
def run_zero_shot(
    model_name,
    data_path,
    output_path,
    ...
    # Set reproducibility configurations
    set_seed(seed)
    ...
    # Initialize dataframe structure columns
    if "llama_raw" not in df.columns:
        df["llama_raw"] = None
    if "llama_pred" not in df.columns:
        df["llama_pred"] = None
    ...
    if resume and os.path.exists(output_path):
        ...
        try:
            ...
            if "llama_pred" in df_results.columns:
                finished_indices = df_results[df_results["llama_pred"].notna()].index
                common_indices = df.index.intersection(finished_indices)
                df.loc[common_indices, "llama_pred"] = df_results.loc[common_indices, "llama_pred"]
                df.loc[common_indices, "llama_raw"] = df_results.loc[common_indices, "llama_raw"]
                print(f"Recovered {len(common_indices)} completed predictions.")
    ...
    remaining_indices = df[df["llama_pred"].isna()].index
    if len(remaining_indices) > 0:
        start_pos = df.index.get_loc(remaining_indices[0])
        start_pos = (start_pos // batch_size) * batch_size
    else:
        start_pos = len(df)

    print(f"Resuming at row position: {start_pos} of {len(df)}")
    if start_pos >= len(df):
    ...
    comments_list = df[comment_col].tolist()
    ...
    for i in tqdm(range(start_pos, len(comments_list), batch_size)):
        ...
        # Update DataFrame configuration using aligned positional indexes
        current_indices = df.index[i : i + len(batch_comments)]
        df.loc[current_indices, "llama_raw"] = batch_raw
        df.loc[current_indices, "llama_pred"] = batch_preds

        # Periodic backup to avoid data loss on crashes
        batch_num = (i - start_pos) // batch_size
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Zero-Shot Regional Bias Annotation using Llama LLMs",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Annotate with Llama-3.1-8B-Instruct
  python src/zero_shot/inference_llama.py \\
      --model meta-llama/Llama-3.1-8B-Instruct \\
      --data_path data/raw/test_comments.csv \\
      --output results/annotation_results_llama_3_1_8b.csv
  ...
  python src/zero_shot/inference_llama.py \\
    ...
    parser.add_argument("--model", type=str, default="meta-llama/Llama-3.1-8B-Instruct", help="HuggingFace model ID")
    ...
    parser.add_argument("--batch_size", type=int, default=24, help="Inference batch size")
