...
# Prepends the system instructions directly into the user role to optimize 
# prompt adherence and satisfy internal formatting requirements of Gemma tokenizers.
...
#
# Args:
#    model_name             - HuggingFace model ID (e.g., 'google/gemma-3-27b-it')
...
def run_zero_shot(
    ...
    if "gemma_raw" not in df.columns:
        df["gemma_raw"] = None
    if "gemma_pred" not in df.columns:
        df["gemma_pred"] = None
    ...
    if resume and os.path.exists(output_path):
        ...
        try:
            try:
                ...
                print("ParserError detected. Repairing...")
                robust_repair_csv(output_path)
                df_results = pd.read_csv(output_path)

            if "gemma_pred" in df_results.columns:
                finished_indices = df_results[df_results["gemma_pred"].notna()].index
                df.loc[finished_indices, "gemma_pred"] = df_results.loc[finished_indices, "gemma_pred"]
                df.loc[finished_indices, "gemma_raw"] = df_results.loc[finished_indices, "gemma_raw"]
    ...
    remaining_indices = df[df["gemma_pred"].isna()].index
    ...
    for i in tqdm(range(start_index, len(comments), batch_size)):
        ...
        df.loc[i : batch_end - 1, "gemma_raw"] = batch_raw
        df.loc[i : batch_end - 1, "gemma_pred"] = batch_preds
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Zero-Shot Regional Bias Annotation using Gemma LLMs",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Annotate with Gemma-3-27b-it
  python src/zero_shot/inference_gemma.py \\
      --model google/gemma-3-27b-it \\
      --data_path data/raw/test_comments.csv \\
      --output results/annotation_results_gemma_27b.csv
  ...
  python src/zero_shot/inference_gemma.py \\
    ...
    parser.add_argument("--model", type=str, default="google/gemma-3-27b-it", help="HuggingFace model ID")
