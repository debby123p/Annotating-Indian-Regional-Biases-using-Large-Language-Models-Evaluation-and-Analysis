...
# Reads a corrupted file as raw text, drops the last line
# (likely incomplete/corrupt), and overwrites the file for recovery.
def repair_csv(file_path):
    ...
    try:
        ...
        if len(lines) > 0:
            print(f"   Removing incomplete last line (Line {len(lines)})...")
            lines = lines[:-1]

        with open(file_path, "w", encoding="utf-8") as f:
            f.writelines(lines)
        print(" Repair complete. Retrying load...")
...
def run_zero_shot(
    ...
    # Load dataset with robust recovery
    ...
    if resume and os.path.exists(output_path):
        ...
        try:
            ...
            repair_csv(output_path)
            try:
                df = pd.read_csv(output_path)
            except Exception as e:
                print(f"CRITICAL ERROR: Could not repair file. Error: {e}")
                print(f"Try using the backup file at: {backup_path}")
                return

        if "qwen_pred" in df.columns:
            processed_count = df["qwen_pred"].notna().sum()
    ...
    for i in tqdm(range(start_index, len(comments), batch_size)):
        ...
        df.loc[i : batch_end - 1, "qwen_raw_output"] = batch_raw
        df.loc[i : batch_end - 1, "qwen_pred"] = batch_preds
...
Examples:
  ...
  python src/zero_shot/inference.py \\
    ...
    parser.add_argument("--model", type=str, default="Qwen/Qwen3-8B", help="HuggingFace model ID")
