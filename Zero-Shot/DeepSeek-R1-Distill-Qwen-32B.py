...
SYSTEM_PROMPT = (
    ...
    "2. Then, output the final classification strictly in this format:\n"
    "    Final Label: [0 or 1]"
)
...
def extract_label(text):
    ...
    # Fallback pattern 1: "Label: 0" or "Label: 1"
    match_fallback = re.search(r"Label\s*:\s*([01])", clean_text, re.IGNORECASE)
    if match_fallback:
        return int(match_fallback.group(1))

    # Fallback pattern 2: Find the last isolated binary digit sequence in output text
    backtrack = re.findall(r"\b([01])\b", clean_text)
    if backtrack:
        return int(backtrack[-1])
...
#
# Args:
#    model_name             - HuggingFace model ID (e.g., 'deepseek-ai/DeepSeek-R1-Distill-Qwen-14B')
...
def run_zero_shot(
    ...
    # Load dataset tracking historical annotations state checkpoints
    if resume and os.path.exists(output_path):
        print(f"Resuming from existing progress file: {output_path}")
        df = pd.read_csv(output_path)
        
        if "deepseek_pred" in df.columns and df["deepseek_pred"].isna().any():
            start_index = df["deepseek_pred"].isna().idxmax()
        elif "deepseek_pred" not in df.columns:
            df["deepseek_raw_output"] = None
            df["deepseek_pred"] = None
            start_index = 0
        else:
            print("All rows are already annotated.")
            start_index = len(df)
    else:
        print(f"Starting new annotation from dataset: {data_path}")
    ...
    if start_index >= len(df):
        ...
        for i in tqdm(range(start_index, len(comments), batch_size)):
            ...
            for idx, response_text in enumerate(generated_texts):
                actual_idx = i + idx
                df.at[actual_idx, "deepseek_raw_output"] = response_text
                df.at[actual_idx, "deepseek_pred"] = extract_label(response_text)
    ...
    # Evaluation summary
    valid_df = df.dropna(subset=["deepseek_pred", label_col])
    if len(valid_df) > 0 and label_col in df.columns:
        y_true = valid_df[label_col].astype(int)
        y_pred = valid_df["deepseek_pred"].astype(int)
        ...
        print(
            classification_report(
                y_true, y_pred, target_names=["Non-Bias (0)", "Bias (1)"], digits=4
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Zero-Shot Regional Bias Annotation using DeepSeek Distilled LLMs",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Annotate with DeepSeek-R1-Distill-Qwen-14B
  python src/zero_shot/inference_deepseek.py \\
      --model deepseek-ai/DeepSeek-R1-Distill-Qwen-14B \\
      --data_path data/raw/test_comments.csv \\
      --output results/annotation_results_deepseek_14b.csv
  ...
  python src/zero_shot/inference_deepseek.py \\
    ...
    parser.add_argument("--model", type=str, default="deepseek-ai/DeepSeek-R1-Distill-Qwen-14B", help="HuggingFace model ID")
