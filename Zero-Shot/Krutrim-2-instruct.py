...
def print_gpu_info(gpu_id=0):
    if torch.cuda.is_available():
        ...
        if vram_gb < 70:
            print("WARNING: You seem to have < 70GB VRAM.")
            print("   The 32B model requires ~65GB in bfloat16.")
            print("   If this crashes, try setting load_in_8bit=True (requires bitsandbytes).")
    else:
...
#
# Args:
#    model_name             - HuggingFace model ID (e.g., 'krutrim-ai-labs/Krutrim-2-instruct')
...
def run_zero_shot(
    ...
    if resume and os.path.exists(output_path):
        ...
        if "krutrim_pred" in df.columns:
            processed_count = df["krutrim_pred"].notna().sum()
        ...
        try:
            ...
            df["krutrim_raw_output"] = None
            df["krutrim_pred"] = None
            start_index = 0
    ...
    for i in tqdm(range(start_index, len(comments), batch_size)):
        ...
        # Drop token type IDs if present to satisfy generative constraints
        ...
        df.loc[i : batch_end - 1, "krutrim_raw_output"] = batch_raw
        df.loc[i : batch_end - 1, "krutrim_pred"] = batch_preds
    ...
    # Evaluation summary
    valid_df = df[df["krutrim_pred"].notna()]
    if len(valid_df) > 0 and label_col in df.columns:
        y_true = valid_df[label_col]
        y_pred = valid_df["krutrim_pred"].astype(int)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Zero-Shot Regional Bias Annotation using Krutrim LLMs",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Annotate with Krutrim-2-instruct
  python src/zero_shot/inference_krutrim.py \\
      --model krutrim-ai-labs/Krutrim-2-instruct \\
      --data_path data/raw/test_comments.csv \\
      --output results/annotation_results_krutrim.csv
  ...
  python src/zero_shot/inference_krutrim.py \\
    ...
    parser.add_argument("--model", type=str, default="krutrim-ai-labs/Krutrim-2-instruct", help="HuggingFace model ID")
